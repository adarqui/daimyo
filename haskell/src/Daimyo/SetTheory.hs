...
oddNats :: (Num a, Enum a) => [a]
oddNats = [1,3..]

evenNats :: (Num a, Enum a) => [a]
evenNats = [0,2..]
...
axiom_of_extensionality _A _B
  | size _A == size _B = and [ member b _A && member a _B | a <- _A'elements , b <- _B'elements]
  | otherwise = False
  where
    _A'elements = elems _A
    _B'elements = elems _B


axiom_of_extensionality' :: (Eq a, Ord a) => Set a -> Set a -> Bool
axiom_of_extensionality' _A _B = elems _A == elems _B


t_proof_axiom_of_extensionality = (and results, results)
  where
...
def_symmetric_difference _A _B =
  ...
  where
  _BArc = theorem_relative_complement _B _A
  _ABrc = theorem_relative_complement _A _B
...
theorem_not_included _A _B = _A `axiom_of_union` _B `axiom_of_extensionality` _B
