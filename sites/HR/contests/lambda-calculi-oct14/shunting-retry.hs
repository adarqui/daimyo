...
--    putStrLn $ show $ reverse x
    putStrLn $ show $ eval'modp $ reverse x
...
eval' acc stack (n@(x:r):xs) =
 case (isntOp n) of
  True -> eval' acc (readDouble n : stack) xs
...
readDouble s = read (filter (/= '+') $ takeWhile (\c -> c == '+' || c == '-' || isDigit c) s) :: Double
...
t9 = eval'modp $ bang "+2-3"
t10 = eval'modp $ bang "-2-3"
