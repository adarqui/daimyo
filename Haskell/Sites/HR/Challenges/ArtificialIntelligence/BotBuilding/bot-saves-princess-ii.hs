...
-
- janking this up based on the previous challenge, to solve bot-saves-princess-ii.hs
-
...
displayPathtoPrincess'' :: Cell -> Cell -> [Move]
...
nextMove rows_cols input =
  let
    grid = overlayGrid (emptyGrid rows_cols) $ concat input
    (Just princess) = findElem Princess grid
    (Just bot) = findElem Bot grid
    moves = displayPathtoPrincess'' bot princess
  in
    gameOutput4HackerRank $ [head moves]
