...
findSmallest'Array'Writer l = do
    (r, w) <- runWriterT (checkList'Writer l)
    let r' = fst $ fromJust $ find (not . snd) $ assocs r
    putStrLn $ "w: " ++ concat w
    return r'
...
checkList'Writer :: [Int] -> WriterT [String] IO (Array Int Bool)
checkList'Writer xs = do
    let n = length xs
    let a = accumArray (||) False (0, n) (zip (filter (<= n) xs) (repeat True))
    tell $ ["length = " ++ show n]
    return a
...
t_findSmallest'Array'Writer = runWriterT (checkList'Writer t_set)
